...
seguridad**). Este documento es la **especificación del formato** y las **instrucciones de
trabajo** para una IA (Claude, ChatGPT, Gemini…): dáselo junto con el catálogo de ejercicios y
tu plan, y te devolverá el fichero listo para importar.

## Cómo usarlo (para personas)

1. Abre un chat con la IA y adjunta (o pega) **dos ficheros de este repo**:
   - `docs/generar-plan-con-ia.md` (este documento).
   - `docs/catalogo-ejercicios.md` (los ~890 ejercicios que la app ya trae, con su `id`).
     **Sin él la IA se inventa ejercicios personalizados para todo** y luego salen sin foto,
     sin instrucciones y duplicados de los que ya existen.
2. Cuéntale tu plan: días que entrenas, material, nivel, objetivo, ejercicios, series,
   repeticiones, pesos de partida y, si quieres, la progresión por semanas (bloques, descargas,
   semanas de test). Si tienes el plan en PDF o foto, adjúntalo también.
3. Pídele algo así:

   > Genera el JSON de copia de seguridad de LiftLog para este plan siguiendo la
   > especificación. Usa solo `exerciseId` del catálogo adjunto; crea ejercicios
   > personalizados únicamente si no existe nada equivalente y dime cuáles has creado y por qué.
   > Al final, dame la tabla de correspondencia ejercicio → id.

4. Guarda la respuesta como `mi-plan.json` (solo el JSON, sin texto alrededor).
5. Opcional pero recomendado: si tienes el repo, valida el fichero:
...
6. Pasa el fichero al móvil (Descargas, Drive, correo…) y en LiftLog:
   **Ajustes → Importar copia de seguridad → elige el fichero → Fusionar**.
   ...
   - **Reemplazar todo** borra rutinas, entrenos y medidas antes de importar. Solo para
     empezar de cero.
7. Comprueba en la pestaña **Entreno** que están las carpetas y rutinas, y abre una para ver la
...
1. **Aclara lo que falte** antes de inventar: días por semana, material disponible, nivel,
   ...
   todo, no preguntes.
2. **Mapea cada ejercicio del plan a un `id` del catálogo** (`docs/catalogo-ejercicios.md`).
